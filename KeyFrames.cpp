...
void saveFrame(void)
{
	...
	//Rotacion cabeza
	KeyFrame[FrameIndex].head = head; // Guarda la rotación actual de la cabeza
	FrameIndex++;
}
...
void resetElements(void)
{
	...
	head = KeyFrame[0].head; // Regresa la cabeza al inici
}
...
void DoMovement()
{
	...
	if (keys[GLFW_KEY_4]) 
	{
		head += 1.0f; // Rota a un lado
	}
	if (keys[GLFW_KEY_5]) 
	{
		head -= 1.0f; // Rota al otro
	}
...
void Animation() {
	...
	if (play)
	{
		...
		else
		{
			...
			// --- APLICAR MOVIMIENTO A LA CABEZA ---
