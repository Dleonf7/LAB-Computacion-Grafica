...
int main() {
 ...
 //     
	...
	//	// Primer cubo escalar 1,1,1   Izquier derecha (Rotacion, escalado, traslacion diferente)
	//	
	//	
	//	
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::rotate(model, 2.0f, glm::vec3(10.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(2.0f, 2.8f, 6.0f)); // ← NUEVA LÍNEA
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
		...
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(2.0f, 0.0f, 0.0f));
		model = glm::rotate(model, 0.5f, glm::vec3(10.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(1.0f, 1.8f, 1.8f)); // ← NUEVA LÍNEA
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
